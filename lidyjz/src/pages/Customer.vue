...
<template>
	<div class="customer">
		...
		<!-- 添加/修改信息对话框 -->
		<el-dialog title="添加/修改 顾客信息" :visible="dialogFormVisible" @close="closeDialogHandler">
		...
		<el-row :gutter="10" style="margin:20px 0;">
			...
			<el-col :span="1">
				<el-button type="primary" @click="loadCustomers(params)">搜索</el-button>
			</el-col>
			<!-- 搜索框和搜索按钮结束 -->
		</el-row>
		<!-- 上方操作栏结束 -->
		<!-- 数据表 -->
		<el-table :data="customers.list" border stripe style="width:100%"  @selection-change="idsChangeHandler">
			<el-table-column
			type="selection">
			</el-table-column>
			<el-table-column
	...
	export default {
		...
		watch:{
			...
			"searchType": function() {
				...
				this.params.status = "";
				// 重载数据
				this.loadCustomers(this.params);
			}
		},
		computed:{
		...
		methods:{
			// 映射vuex中动作区的方法
			...mapActions("customer", ["loadCustomers", "deleteCustomer", "batchDeleteCustomers", "saveCustomer"]),
			// 信息对话框关闭方法
			closeDialogHandler() {
				// 关闭信息对话框
